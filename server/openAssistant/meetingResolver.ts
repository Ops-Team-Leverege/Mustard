...
export async function searchAcrossMeetings(
  ...
  const { executeSingleMeetingContract } = await import("./singleMeeting");
  ...
  const results = await Promise.all(
    meetingsToSearch.map(async (meeting) => {
      try {
        ...
        const result = await executeSingleMeetingContract(meeting, userMessage, AnswerContract.EXTRACTIVE_FACT, undefined);
