...
export function getSourceAttribution(ctx: SourceAttributionContext): string {
  ...
  if (dataSource === "qa_pairs" && SOURCE_ATTRIBUTION_CONFIG.transcripts_artifacts) {
    return `\n\n_Source: Meeting Customer Questions_`;
  }

  const transcriptSources = ["transcript", "summary", "binary_answer", "action_items", "attendees", "meeting_artifacts"];
  ...
  if (dataSource === "general_knowledge" && SOURCE_ATTRIBUTION_CONFIG.general_knowledge) {
    const model = usedSingleMeetingMode
      ...
      : friendlyModelName(MODEL_ASSIGNMENTS.GENERAL_HELP_RESPONSE);
    return `\n\n_Source: Generated by ${model}_`;
  }

  if ((intent === "GENERAL_HELP" || intent === "GREETING") && SOURCE_ATTRIBUTION_CONFIG.general_knowledge) {
    const model = friendlyModelName(MODEL_ASSIGNMENTS.GENERAL_HELP_RESPONSE);
    return `\n\n_Source: Generated by ${model}_`;
  }
