...
/**
 * Commitment-based next step extracted from meeting.
 * Similar to Google Meet's autogenerated action items.
 */
...
/**
 * Extract commitment-based next steps from a meeting transcript.
 * 
 * This mirrors Google Meet's autogenerated action items.
 * Only explicit, speaker-grounded commitments are returned.
 * 
 * Principles:
...
If no clear commitments exist, return an empty array: []
        `.trim(),
      },
      {
        role: "user",
...
${transcript}
  ...
  const content = response.choices[0]?.message?.content;
  if (!content) {
    throw new Error("LLM returned empty commitment extraction");
  }
