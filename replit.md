...
- **Jira Roadmap Integration**: Dedicated Roadmap page (`/roadmap`) integrates with Jira to display issues from multiple configured projects. Users can configure Jira project keys (e.g., "PROJ", "MYAPP") via a settings dialog, and the system fetches up to 100 issues across all configured projects. Issues are displayed grouped by project with status badges (color-coded: Done=green, In Progress=blue), issue type icons, priority indicators, assignee avatars, and direct links to Jira. Project configuration is stored per-user in the `users.jiraProjectKeys` array field. Backend uses Replit's Jira connection (OAuth2) for secure API access via jira.js client library.
