...
-   **customer_questions Table Removed**: The `customer_questions` table, extraction pipeline, and all references have been fully removed. All Q&A retrieval now uses `qa_pairs` exclusively, which provides better coverage (100% answer rate vs 59%) through GPT-5 interpreted extraction.
...
-   **Multi-Source Search Pattern (Slack Search Offer)**: After every meeting-based response (SINGLE_MEETING or MULTI_MEETING), the bot appends an offer to also search Slack for internal discussions. Uses the existing `pendingOffer` system — stores `"slack_search"` in interaction metadata's `resolution` JSON. When the user accepts (affirmative reply), the clarification handler routes through OpenAssistant with a synthetic SLACK_SEARCH Decision Layer result, maintaining the Decision Layer as sole authority. The offer is generic (not hardcoded to specific topics) and the Slack search handler has graceful error handling with user-facing fallback message.
-   **Customer-Specific Deployment Routing**: Intent classification rule #6 ensures questions mentioning a customer name alongside deployment terms (cameras, hardware, installation) route to SINGLE_MEETING instead of PRODUCT_KNOWLEDGE, since the user is asking about that customer's specific PitCrew setup discussed in meetings.
-   **Aggregate Fallback (qa_pairs_first)**: When a MULTI_MEETING aggregate query has no customer scope and no time range (would previously CLARIFY), the system now searches `qa_pairs` by keyword first and returns immediate results. The Decision Layer sets `aggregateFallback: "qa_pairs_first"` instead of returning CLARIFY intent. Events.ts intercepts this flag, runs `storage.searchQaPairsByKeyword()`, formats results grouped by company, and appends an offer to search meetings (with time range) or Slack. Follow-up handling: "check Slack" → routes through existing Slack search handler with synthetic DL result; generic "yes" → guides user to specify scope; time-range reply → flows through normal Decision Layer (time range now present, no fallback triggers). Uses `pendingOffer: "meeting_search"` in interaction metadata.
...
-   **Authentication**: Header `X-Airtable-Secret` (uses `AIRTABLE_WEBHOOK_ZAPIER` secret)
