...
- **Supporting Materials**: Transcript upload form includes an optional "Supporting Materials" section where users can upload additional documents (via file upload or URL fetch) that supplement the call transcript or meeting notes. Supported file formats include .txt, .docx, and .pdf. Supporting materials are stored in the `transcripts.supportingMaterials` field and included in the AI analysis prompt alongside the main transcript/notes content, enabling the AI to extract insights from technical documents, presentation decks, product specifications, or other materials that weren't fully captured in the conversation.
